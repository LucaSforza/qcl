...
pub fn history_add(line: &str) -> i32 {
    // Keep the CString alive until the native call has consumed its pointer.
    let cs = CString::new(line).unwrap();
    unsafe { ffi::linenoiseHistoryAdd(cs.as_ptr()) }
}
...
pub fn history_save(file: &str) -> i32 {
    ...
    unsafe { ffi::linenoiseHistorySave(fname.as_ptr()) }
}
...
pub fn history_load(file: &str) -> i32 {
    // Keep the CString alive until the native call has consumed its pointer.
    let fname = CString::new(file).unwrap();
    unsafe { ffi::linenoiseHistoryLoad(fname.as_ptr()) }
}
...
pub fn add_completion(c: *mut Completions, s: &str) {
    // The native function copies the string during this call.
    let completion = CString::new(s).unwrap();
    unsafe {
        ffi::linenoiseAddCompletion(c, completion.as_ptr());
    }
}
